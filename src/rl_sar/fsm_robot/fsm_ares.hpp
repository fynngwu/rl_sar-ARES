 ...
 * Adapted for ARES quadruped from fsm_uika.hpp
 */
...
public:
    ...
    void Run() override
    {
        for (int i = 0; i < rl.params.Get<int>("num_of_dofs"); ++i)
        {
            ...
            fsm_command->motor_command.kd[i] = 0.5f;
...
public:
    ...
    // ARES default standing pose — all zeros because driver handles motor offsets
...
class ARESFSMFactory : public FSMFactory
{
public:
    ARESFSMFactory(const std::string& initial) : initial_state_(initial) {}
    std::shared_ptr<FSMState> CreateState(void *context, const std::string &state_name) override
    {
        RL *rl = static_cast<RL *>(context);
        if (state_name == "RLFSMStatePassive")
            return std::make_shared<ares_fsm::RLFSMStatePassive>(rl);
        else if (state_name == "RLFSMStateGetUp")
            return std::make_shared<ares_fsm::RLFSMStateGetUp>(rl);
        else if (state_name == "RLFSMStateGetDown")
            return std::make_shared<ares_fsm::RLFSMStateGetDown>(rl);
        else if (state_name == "RLFSMStateRLLocomotion")
            return std::make_shared<ares_fsm::RLFSMStateRLLocomotion>(rl);
        return nullptr;
    }
    std::string GetType() const override { return "ares"; }
...
REGISTER_FSM_FACTORY(ARESFSMFactory, "RLFSMStatePassive")
